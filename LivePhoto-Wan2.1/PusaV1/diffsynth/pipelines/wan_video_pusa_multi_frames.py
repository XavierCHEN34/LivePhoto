...
class PusaMultiFramesPipeline(BasePipeline):
    ...
    def __call__(
        ...
        if type(motion_intensity) is int:
            motion_intensity = torch.Tensor([motion_intensity]).to(dtype=self.torch_dtype, device=self.device)
        ...
        for progress_id, timestep in enumerate(progress_bar_cmd(self.scheduler.timesteps)):
            ...
            noise_pred_posi = model_fn_wan_video(
                ...
                x=latents, timestep=timestep, motion_intensity=motion_intensity,
            ...
            if cfg_scale != 1.0:
                noise_pred_nega = model_fn_wan_video(
                    self.dit, motion_controller=self.motion_controller, vace=self.vace,
                    x=latents, timestep=timestep, motion_intensity=0,
                    **prompt_emb_nega, **image_emb, **extra_input,
                    **tea_cache_nega, **usp_kwargs, **motion_kwargs, **vace_kwargs,
                )
                noise_pred_empty = model_fn_wan_video(
                    self.dit, motion_controller=self.motion_controller, vace=self.vace,
                    x=latents, timestep=timestep, motion_intensity=0,
                    **prompt_emb_posi, **image_emb, **extra_input,
                    **tea_cache_nega, **usp_kwargs, **motion_kwargs, **vace_kwargs,
                )
                noise_pred = noise_pred_nega + cfg_scale * (noise_pred_posi - noise_pred_nega) + cfg_scale * (noise_pred_posi - noise_pred_empty)
            else:
...
def model_fn_wan_video(
    ...
    motion_intensity: Optional[torch.Tensor] = None,
    **kwargs,
):
    ...
    if type(motion_intensity) is int:
        motion_intensity = torch.Tensor([motion_intensity]).to(dtype=int, device="cpu")
    
    motion_intensity_emb = dit.motion_intensity_embed(motion_intensity.to(dtype=int, device="cpu"))
    motion_intensity_emb = motion_intensity_emb.to(device="cuda").reshape(motion_intensity_emb.shape[0], 2, -1)
    context = dit.text_reweight(context, motion_intensity_emb)
