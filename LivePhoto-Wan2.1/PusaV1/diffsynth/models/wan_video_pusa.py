...
def rope_apply(x, freqs, num_heads):
    ...
    if freqs.ndim == 2:
        freqs = freqs[:x.shape[1]].unsqueeze(0).unsqueeze(2)  # (1, s, 1, d)
...
class TextReweight(torch.nn.Module):
    def __init__(self, text_dim: int, freqs: Optional[torch.Tensor] = None):
        super().__init__()
        self.dim = text_dim
        self.cross_attn_0 = CrossAttention(text_dim, num_heads=8, eps=1e-6, has_image_input=False)
        self.cross_attn_1 = CrossAttention(text_dim, num_heads=8, eps=1e-6, has_image_input=False)

        self.ln0_0 = nn.LayerNorm(text_dim, eps=1e-6)
        self.ln0_1 = nn.LayerNorm(text_dim, eps=1e-6)
        self.ln1_1 = nn.LayerNorm(text_dim, eps=1e-6)
        self.warmup = nn.Parameter(torch.tensor(-9.0))
        
        head_dim = text_dim // 8
        self.freqs = precompute_freqs_cis(head_dim)

    
    def forward(self, prompt_emb, motion_emb):
        # prompt_emb: (B, N, D)
        # motion_emb: (B, 2, D)

        x = prompt_emb

        x = x + self.cross_attn_0(self.ln0_1(x), motion_emb)
        x = x + self.cross_attn_1(self.ln1_1(x), motion_emb)

        alpha = torch.clamp(F.relu(self.warmup), 0.0, 1.0)

        prompt_emb = (1 - alpha) * prompt_emb + alpha * self.ln0_0(x)
...
class WanModelPusa(torch.nn.Module):
    def __init__(
        ...
        self.motion_intensity_embed = nn.Embedding(7, text_dim * 2)
        self.text_reweight = TextReweight(text_dim, self.freqs)
    ...
    def forward(self,
                ...
                motion_intensity: Optional[torch.Tensor] = torch.Tensor([4]),
                **kwargs,
                ):
        # print(x)
        ...
        motion_emb = self.motion_intensity_embed(motion_intensity.to(torch.int64)).to(x.dtype).view(B, 2, -1)

        context = self.text_reweight(context, motion_emb)
...
class WanModelPusaStateDictConverter:
    ...
    def from_civitai(self, state_dict):
        ...
        if hash_state_dict_keys(state_dict) == "71bb75b826bdceb62022846bbaabfa9d":
            config = {
